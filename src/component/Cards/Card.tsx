    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 p-6 container mx-auto">
      {cards.map((card: Icard) => {
        return (
          <div
            key={card.id}
            className="bg-white border border-gray-200 rounded-3xl p-6 flex flex-col shadow-sm"
          >
            ...
            <button className="w-full bg-[#0F172A] hover:bg-black text-white font-medium py-3 rounded-xl transition-colors">
              Add to Stack
            </button>
          </div>
        );
